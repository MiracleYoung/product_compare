...
def feiniu(product_name):
    # 飞牛网
    result = feiniu_product(product_name, session)
    return jsonify(result)

@app.route('/product/jd/<product_name>', methods=['GET'])
def jd(product_name):
    # 京东
    result = jd_product(product_name, session)
...
def tmall(product_name):
    # 天猫超市
    result = tmall_product(product_name, session)
    return jsonify(result)

@app.route('/product/auchan/<product_name>', methods=['GET'])
def auchan(product_name):
    # 欧尚
    result = auchan_product(product_name, session)
    return jsonify(result)
